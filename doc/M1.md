...
| T04  | Dokumentation Meilenstein 1                                  | M1 | Lena | ![80%](https://progress-bar.dev/80) |
| T05  | Schnittstellendesign                                         | M1 | Oli & Lena | ![70%](https://progress-bar.dev/70) |
...
#### Einschätzung
Mit den oben genannten Punkten müsste das Komplexitätsniveau hoch genug sein um hier volle Punktzahl erreichen zu können.
...
das Backend weitergeleitet. Im Backend wird der Status von Spielen zentral verwaltet. Alle Änderungen, die durch den Client (oder die KI)
erfolgen, werden hier validiert und danach ausgeführt. Schliesslich wird der Spielstatus wieder an den Client geschickt. Der Client
reagiert, indem die Änderungen durch Rendering visualisiert werden.  
Zu diesem Zeitpunkt ist noch nicht klar, ob das Backend mit einer Persistenzschicht verbunden wird. Wenn ja, würden die Daten (Models) via
Repository mit einer SQL-Datenbank synchronisiert.

### Protokoll
Die Kommunikation soll über ein menschenlesbares Webservice-Protokoll stattfinden. Diese Variante ist konzeptuell sinnvoll,
weil zu Beginn eine Verbindung aufgebaut und über das ganze Spiel erhalten werden kann.  
...
Die Verbindung wird mit einem Client-Aufruf von ``ws://localhost:8080/ws`` automatisch angefragt, sobald ein User die Seite betritt.
<table>
  <tr>
    <td><b>Sender</b></td>
    <td><b>Receiver</b></td>
  ...
  <tr>
    <td colspan="4"><pre><code>CONNECT
accept-version:1.0,1.1,1.2
heart-beat:10000,10000</code></pre></td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <td colspan="4"><pre><code>CONNECTED
version:1.2
heart-beat:0,0</code></pre></td>
  </tr>
</table>

##### 3. Beenden der Verbindung
Das Beenden einer Verbindung wird über das Framework verwaltet. Dies, weil eine Verbindung durch explizites Beenden, aber auch durch
sonstige Verbindungsabbrüche (z.B. Schliessen des Clients) geschlossen werden kann.
...
Die Websocket wird 'abonniert', der Client wird somit automatisch
vom Server über Änderungen informiert.
<table>
  <tr>
    <td><b>Sender</b></td>
    <td><b>Receiver</b></td>
  ...
  <tr>
    <td colspan="4"><pre><code>SUBSCRIBE
id:sub-1
destination:/user/topic/user</code></pre></td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <td colspan="4"><pre><code>SEND
destination:/app/user/join</code></pre></td>
  </tr>
</table>
...
Der Client kann die Liste verwenden, um die für ein Spiel verfügbaren User darzustellen oder ggf. eine Highscore anzuzeigen.  
...
<table>
  ...
  <tr>
    <td colspan="4"><pre><code>SUBSCRIBE
id:sub-2
destination:/user/topic/gamerequest</code></pre></td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <td colspan="4"><pre><code>SEND
destination:/app/gamerequest/join</code></pre></td>
  </tr>
</table>
...
Erhält ein User eine Einladung, wird im die ID des Einladenden übermittelt. Der Client hat damit das Signal, das Spiel zu initialisieren.
...
<table>
  ...
  <tr>
    <td colspan="4"><pre><code>SUBSCRIBE
id:sub-3
destination:/user/topic/game</code></pre></td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <td colspan="4"><pre><code>SEND
destination:/app/game/join</code></pre></td>
  </tr>
</table>
...
- ``x``: die x-Position
- ``y``: die y-Position
- ``state``: der Status eines Objektes, codiert als Integer (z.B. nach rechts orientiert, springend)
